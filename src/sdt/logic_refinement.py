...
# Lazy import to avoid circular dependency; resolved inside method.
_dataset_rules_module = None

def _get_dataset_rules_module():
    global _dataset_rules_module
    if _dataset_rules_module is None:
        from src.sdt import dataset_rules as _mod
        _dataset_rules_module = _mod
    return _dataset_rules_module
...
class OntologyRefinementGenerator:
    ...
    Supports dataset-specific prioritization of refinements via dataset_concepts parameter.
    """
    def __init__(self, ontology_manager, static_refinements: Optional[List[OntologyRefinement]] = None, 
                 dl_profile: str = 'ALC', dataset_name: Optional[str] = None, 
                 dataset_concepts: Optional[Dict[str, List]] = None,
                 dataset_refinement_profile: Optional[Dict[str, Any]] = None,
        ...
        # Dataset-specific context: e.g., {'GO': ['GO_0003677', ...], 'DTO': [...], ...}
        ...
        self.reasoner_engine = str(reasoner_engine or 'none').lower()
        self.reasoner_infer_property_values = bool(reasoner_infer_property_values)
        self._reasoner_initialized = False
        self._reasoner_shared_state = self._get_reasoner_shared_state()

        # Safety knobs to avoid refinement explosion on large ontologies (e.g., DTO)
        self.max_qualification_concepts_per_property = int(
            self.dataset_refinement_profile.get('max_qualification_concepts_per_property', 50)
        )
        ...
        profile_ref_types = self.dataset_refinement_profile.get('allowed_ref_types', None)
        if isinstance(profile_ref_types, list) and profile_ref_types:
            self.allowed_ref_types = [str(x).strip().lower() for x in profile_ref_types if str(x).strip()]

        self.allowed_object_properties = {
            str(x).strip()
            for x in (self.dataset_refinement_profile.get('allowed_object_properties') or [])
            if str(x).strip()
        }
        self.allowed_data_properties = {
            str(x).strip()
            for x in (self.dataset_refinement_profile.get('allowed_data_properties') or [])
            if str(x).strip()
        }

    def _get_reasoner_shared_state(self) -> Dict[str, bool]:
        """Get per-ontology-manager shared reasoner state.

        This allows one-time reasoner execution per task even when multiple
        learners/generators are created (e.g., bagging trees).
        """
        state = getattr(self.ontology_manager, '_semantic_reasoner_state', None)
        if not isinstance(state, dict):
            state = {}
            setattr(self.ontology_manager, '_semantic_reasoner_state', state)
        return state

    def _run_reasoner_once(self) -> None:
        """Run ontology reasoner once per generator instance.

        - none: skip
        - hermit: owlready2 sync_reasoner (HermiT)
        - pellet: owlready2 sync_reasoner_pellet
        """
        if self._reasoner_initialized:
            return

        engine = self.reasoner_engine
        state_key = f"{engine}|infer={int(self.reasoner_infer_property_values)}"

        if self._reasoner_shared_state.get(state_key, False):
            self._reasoner_initialized = True
            return

        if engine in ('none', 'off', 'disabled'):
            self._reasoner_shared_state[state_key] = True
            self._reasoner_initialized = True
            return

        try:
            def _call_reasoner(reasoner_fn):
                kwargs = {
                    'infer_property_values': self.reasoner_infer_property_values,
                ...
                try:
                    ...
                    if 'infer_data_property_values' in str(te):
                        kwargs.pop('infer_data_property_values', None)
                        reasoner_fn(**kwargs)
                    else:
                        raise
            ...
            with self.onto:
                if engine == 'pellet':
                    _call_reasoner(sync_reasoner_pellet)
                else:
                    # default to HermiT path
                    _call_reasoner(sync_reasoner)
        except Exception as e:
            # Keep training robust if Java / reasoner runtime is unavailable.
            print(f"[WARN] Reasoner '{engine}' failed; continuing without reasoner: {e}")
            msg = str(e).lower()
            if 'java heap space' in msg or 'outofmemoryerror' in msg:
                java_opts = os.environ.get('JAVA_TOOL_OPTIONS', '').strip()
                if not java_opts:
                    print(
                        "[HINT] HermiT JVM memory seems too small. "
                        "Set JAVA_TOOL_OPTIONS='-Xmx8g' (or higher, e.g. -Xmx16g) before running."
                    )
                else:
                    print(
                        f"[HINT] Current JAVA_TOOL_OPTIONS='{java_opts}'. "
                        "Increase -Xmx (e.g. -Xmx16g) if OOM persists."
                    )
        finally:
            self._reasoner_shared_state[state_key] = True
            self._reasoner_initialized = True
    ...
    def _get_relevant_object_properties(self, center_class, instances: List):
        ...
        for prop in self.onto.object_properties():
            if self.allowed_object_properties and prop.name not in self.allowed_object_properties:
                continue
    ...
    def _get_relevant_data_properties(self, center_class, instances: List):
        ...
        for prop in self.onto.data_properties():
            if prop.name == 'hasLabel':
                continue
            if self.allowed_data_properties and prop.name not in self.allowed_data_properties:
                continue
            if not self._is_applicable_domain(prop, center_class):
                continue
            if (
    ...
    def _is_dataset_relevant_concept(self, concept) -> bool:
        """Check if concept is relevant to current dataset."""
        if not self.dataset_concepts:
            return True  # If no dataset context, all concepts are relevant
        
        concept_name = str(getattr(concept, 'name', concept))
        
        # Check if concept appears in any dataset-specific concept list
        for onto_name, concept_list in self.dataset_concepts.items():
            if concept_name in concept_list:
                return True
            # Also check if concept matches by containing the name
            for c in concept_list:
                if str(c).lower() in concept_name.lower() or concept_name.lower() in str(c).lower():
                    return True
        
        return False

    def _prioritize_dataset_concepts(self, concepts: List) -> List:
        """Sort concepts to prioritize dataset-relevant ones."""
        if not self.dataset_concepts:
            return concepts
        ...
        for concept in concepts:
            if self._is_dataset_relevant_concept(concept):
                dataset_relevant.append(concept)
            else:
                other.append(concept)
        
        # Return dataset-relevant first, then others
        return dataset_relevant + other

    def _is_toxicity_dataset(self) -> bool:
        name = str(self.dataset_name or '').lower()
        return name in {'tox21', 'clintox', 'sider', 'hiv'}

    def _deduplicate_refinements(self, refinements: List[OntologyRefinement]) -> List[OntologyRefinement]:
        seen = set()
        deduped: List[OntologyRefinement] = []
    ...
    def _generate_toxicity_seed_refinements(self) -> List[OntologyRefinement]:
        """Legacy wrapper – delegates to dataset-specific seed rules."""
        return self._generate_dataset_seed_refinements()

    def _generate_dataset_seed_refinements(self) -> List[OntologyRefinement]:
        """Generate dataset-specific seed refinements as high-priority candidates.

        Each dataset receives up to 15 expert-designed rules reflecting its
        domain knowledge (physicochemical thresholds, structural alerts,
        pharmacophore patterns).

        Controlled by `enable_toxicity_seed_rules` flag (now generalised to
        all datasets) or falls back when dataset_name is set.
        """
        name = str(self.dataset_name or '').strip().lower()
        if not name:
            return []

        # Only inject seeds when explicitly enabled or for known datasets
        if not self.enable_toxicity_seed_rules and name not in (
            'bbbp', 'bace', 'clintox', 'hiv', 'tox21', 'sider',
        ):
            return []

        try:
            mod = _get_dataset_rules_module()
            rules = mod.get_dataset_seed_rules(name, self.onto)
        except Exception as e:
            print(f"[WARN] Failed to load dataset seed rules for '{name}': {e}")
            rules = []

        if rules:
            print(f"[INFO] Injected {len(rules)} dataset-specific seed rules for '{name}'")
        return rules
    ...
    def generate_refinements(
        ...
        """
        # Ensure reasoner materializes inferred class memberships before candidate generation.
        self._run_reasoner_once()
        ...
        # 0. Dataset-specific seed rules (high priority domain knowledge)
        refinements.extend(self._generate_dataset_seed_refinements())
        ...
        refinements = self._deduplicate_refinements(refinements)
    ...
    def _generate_concept_refinements(
        ...
        refs = []
        try:
            subclasses = list(center_class.subclasses())
            # Prioritize dataset-relevant subclasses
            subclasses = self._prioritize_dataset_concepts(subclasses)
    ...
    def _generate_qualification_refinements(self, center_class, instances: List) -> List[OntologyRefinement]:
        ...
        Prioritizes dataset-relevant concepts if dataset context is provided.
        """
        ...
        for prop in target_props:
            ...
            # Prioritize dataset-relevant concepts
            concepts_list = list(observed_concepts)
            concepts_list = self._prioritize_dataset_concepts(concepts_list)
            concepts_sorted = sorted(
                concepts_list,
                key=lambda c: (0 if self._is_dataset_relevant_concept(c) else 1,  # dataset-relevant first
                              str(getattr(c, 'name', '')))
            )
